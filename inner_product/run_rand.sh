...
sudo docker run -v "$PWD":/workdir -e VFC_BACKENDS="libinterflop_ieee.so" verificarlo/verificarlo ./ref 0 $i rand_vec/v_$i.dat ref/ref_r_$i.dat

# UR results
sudo docker run -v "$PWD":/workdir -e VFC_BACKENDS="libinterflop_ieee.so" verificarlo/verificarlo ./UR 0 $i rand_vec/v_$i.dat UR/UR_r_$i.dat
...
sudo docker run -v "$PWD":/workdir -e VFC_BACKENDS="libinterflop_ieee.so" verificarlo/verificarlo ./UR_err UR/UR_r_$i.dat ref/ref_r_$i.dat UR/UR_r_$i.err
    ...
    do
    # SR results
    sudo docker run -v "$PWD":/workdir -e VFC_BACKENDS="libinterflop_mca_int.so --mode=rr" verificarlo/verificarlo ./SR 0 $i rand_vec/v_$i.dat SR/SR_r_$i_$k.dat
