...
int main() 
{
    FILE *file1 = fopen("rapid_1.11_UR.dat", "r");
    if (file1 == NULL) 
    {
    ...
    FILE *file3 = fopen("error_rec_1.11_double_UR.dat", "w");
    if (file3 == NULL) 
    {
        perror("Error opening error_rec_1,11_double_UR.dat");
        fclose(file1);
        fclose(file2);
        return 1;
    }
    ...
    while (fgets(line1, sizeof(line1), file1) != NULL && fgets(line2, sizeof(line2), file2) != NULL && fgets(line3, sizeof(line3), file2) != NULL)
    {
        ...
        if (sscanf(line1, "%*s %lf", &value1) == 1 && sscanf(line2, "%*s %lf", &value2) == 1 && sscanf(line2, "%d", &value3) == 1)
        {
