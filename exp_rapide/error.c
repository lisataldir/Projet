...
int main(int argc, char ** argv) 
{
    ...
    double res_iter, ref_iter, res_rec, ref_rec;
    int nb;
    ...
    while (fgets(line1, sizeof(line1), result) != NULL && fgets(line2, sizeof(line2), reference) != NULL)
    {
        ...
        if (sscanf(line1, "%d", &nb) == 1 && sscanf(line1, "%*s %*s %lf", &res_iter) == 1 && sscanf(line2, "%*s %lf", &ref_iter) == 1 && sscanf(line1, "%*s %lf", &res_rec) == 1 && sscanf(line2, "%*s %lf", &ref_rec) == 1)
        {
            // Calculate the absolute difference
            double err_iter = fabs( (res_iter - ref_iter) / ref_iter );
            double err_rec = fabs( (res_rec - ref_rec) / ref_rec );
