...
paste $(for i in {1..80}; do echo -n "results$k/naif_rec/SR_$i.dat "; done) > results$k/naif_rec/SR.dat
...
paste $(for i in {1..80}; do echo -n "results$k/naif_iter/SR_$i.dat "; done) > results$k/naif_iter/SR.dat
awk '{
    sum = 0;
    for (i = 1; i <= NF; i++) {
        sum += $i;
...
paste $(for i in {1..80}; do echo -n "results$k/rapide_rec/SR_$i.dat "; done) > results$k/rapide_rec/SR.dat
awk '{
    sum = 0;
    for (i = 1; i <= NF; i++) {
        sum += $i;
...
paste $(for i in {1..80}; do echo -n "results$k/rapide_iter/SR_$i.dat "; done) > results$k/rapide_iter/SR.dat
awk '{
    sum = 0;
    for (i = 1; i <= NF; i++) {
        sum += $i;
