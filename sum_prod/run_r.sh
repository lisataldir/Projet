...
sudo docker run -v "$PWD":/workdir -e VFC_BACKENDS="libinterflop_ieee.so" verificarlo/verificarlo ./vec_gen $i random/v_$i.dat
    ...
    sudo docker run -v "$PWD":/workdir -e VFC_BACKENDS="libinterflop_ieee.so" verificarlo/verificarlo ./main 0 $j $i random/v_$i.dat >> UR/UR_r\_$j.err

        for k in {1..10}
        do
        # SR errors
        sudo docker run -v "$PWD":/workdir -e VFC_BACKENDS="libinterflop_mca_int.so --mode=rr" verificarlo/verificarlo ./main 0 $j $i random/v_$i.dat >> SR/SR_r_$i\_$j.err       
        done 
